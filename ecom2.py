...
#Create Time Series Sales visualization
...
with st.expander("TimeSeries_View Data:"):
    # Group by month_year and sum the sales_per_order
    linechart = pd.DataFrame(filtered_df.groupby(filtered_df['month_year'].dt.strftime('%d-%b-%Y'))['sales_per_order'].sum()).reset_index()

    # Sort the DataFrame by month_year
    linechart['month_year'] = pd.to_datetime(linechart['month_year'], format='%d-%b-%Y')
    linechart = linechart.sort_values('month_year')

    # Convert month_year back to the desired format
    linechart['month_year'] = linechart['month_year'].dt.strftime('%d-%b-%Y')
    
     # Format sales_per_order column to display only two decimal places
    linechart['sales_per_order'] = linechart['sales_per_order'].round(2)
